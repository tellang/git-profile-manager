...
function Show-InteractiveMenu {
    ...
    while ($true) {
        ...
        switch ($key.VirtualKeyCode) {
            ...
            13 { return $Options[$selectedIndex] }
            27 { return $null }
        }
    }
}
...
while ($true) {
    ...
    $choice = Show-InteractiveMenu -Title "Git 프로필 관리자 (v15)" -Options $mainMenuOptions -LastProfile $lastAppliedProfile
    ...
    if ($choice -ne "직전 설정으로 복구" -and $choice -ne "종료" -and $choice -ne "현재 설정 보기") {
        ...
        $backupName = git config --global user.name
        $backupEmail = git config --global user.email
        if ($backupName -and $backupEmail) {
            $backupContent = "user.name=$backupName`nuser.email=$backupEmail"
    ...
    switch ($choice) {
        ...
        "현재 설정을 새 프로필로 저장" {
            $profilesDir = "profiles"
            if (-not (Test-Path $profilesDir)) { New-Item -ItemType Directory -Path $profilesDir | Out-Null }
            Write-Host "`n현재 설정을 저장합니다..."
            $currentName = git config --global user.name; $currentEmail = git config --global user.email
            if (-not $currentName -or -not $currentEmail) { Write-Host "❌ 저장할 이름 또는 이메일 설정이 없습니다." -ForegroundColor Red; break }
            Write-Host "  현재 이름: $currentName"; Write-Host "  현재 이메일: $currentEmail"
            ...
            $newProfileContent = "user.name=$currentName`nuser.email=$currentEmail"
            Set-Content -Path $filePath -Value $newProfileContent
            Write-Host "✅ '$profileName.conf' 이름으로 프로필을 '$profilesDir' 폴더에 저장했습니다." -ForegroundColor Green
        }
        "직전 설정으로 복구" {
            $profilesDir = "profiles"
        ...
        "Git 글로벌 설정 초기화" {
            ...
            git config --global --unset-all user.name 2>$null; git config --global --unset-all user.email 2>$null
